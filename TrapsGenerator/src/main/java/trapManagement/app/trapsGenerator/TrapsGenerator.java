...
public class TrapsGenerator {
	...
	private enum ServerType {HTTP, FTP, COMMON};
	...
	public void setFakeUsers(ArrayList<FakeUser> httpUsers, ArrayList<FakeUser> ftpUsers, ArrayList<FakeUser> commonUsers) {
...
//			userNameUrlTextFile();
	...
	private Document manipulateUserNameAndPasswordInHtmlFileById(String srcFileName, String userNameId, String pwdId) throws IOException {
		File htmlSrcFile = openFile(srcFileName);
	...
	private void htmlPasswordTypeExplotation() throws IOException {
		Document doc = manipulateUserNameAndPasswordInHtmlFileById(this.SOURCES_DIR + this.HTML_PWD_TYPE_TRAP, "userNameInput", "pwdInput");
	...
	private void userNameUrlTextFile() throws IOException {
		//		File txtFile = new File(this.USERNAME_URL_FILE);	
		//		if(!txtFile.exists())
		...
		//		pw.println(getFakeUser(ServerType.COMMON));
		//		pw.println(url);
		//		pw.close();
...
//		FileOutputStream out = new FileOutputStream(TARGET_DIR + ZIPPED_IMAGE_PATH + ZIP_FINAL_NAME);
